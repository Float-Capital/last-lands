...
let eventRouter = (item: Types.eventRouterEventAndContext, ~inMemoryStore) => {
  ...
  switch event {
  | NounsTokenContract_NounCreatedWithContext(event, getContext) => {
      event->updateEventSyncState(~chainId, ~inMemoryStore)

      let jsonSerializedParams =
        event.params->Types.NounsTokenContract.NounCreatedEvent.eventArgs_encode

      event->addEventToRawEvents(
        ~inMemoryStore,
        ~chainId,
        ~jsonSerializedParams,
        ~eventName=NounsToken_NounCreated,
      )

      let handler = Handlers.NounsTokenContract.NounCreated.getHandler()
      ...
      | userCodeException =>
        let errorObj =
          userCodeException->getEventExnObj(~event, ~chainId, ~eventName="NounsToken.NounCreated")
  ...
  | Some(lastItemInBatch) =>
    ...
    for i in 0 to eventBatch->Belt.Array.length - 1 {
      ...
      ->Js.Array2.push({
        ...
        logIndex,
        data: switch event {
        | NounsTokenContract_NounCreated(event) => {
            let contextHelper = Context.NounsTokenContract.NounCreatedEvent.contextCreator(
              ~inMemoryStore,
              ~chainId,
              ~event,
              ~logger,
            ...
            let loader = Handlers.NounsTokenContract.NounCreated.getLoader()
            ...
            | userCodeException =>
              let errorObj =
                userCodeException->getEventExnObj(
                  ~event,
                  ~chainId,
                  ~eventName="NounsToken.NounCreated",
                )
            ...
            (
              ...
              Types.NounsTokenContract_NounCreatedWithContext(event, context),
